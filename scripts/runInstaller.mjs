...
const BASE_URL = "https://github.com/CometCord/Installer/releases/latest/download/";
const INSTALLER_PATH_DARWIN = "CometCordInstaller.app/Contents/MacOS/CometCordInstaller";
...
function getFilename() {
    switch (process.platform) {
        case "win32":
            return "CometCordInstallerCli.exe";
        case "darwin":
            return "CometCordInstaller.MacOS.zip";
        case "linux":
            return "CometCordInstallerCli-linux";
        default:
...
async function ensureBinary() {
    ...
    const outputFile = process.platform === "darwin"
        ? join(FILE_DIR, "CometCordInstaller")
        : downloadName;
    ...
    const res = await fetch(BASE_URL + filename, {
        headers: {
            "User-Agent": "CometCord (https://github.com/Vendicated/CometCord)",
            "If-None-Match": etag
        }
    });
    ...
    if (process.platform === "darwin") {
        ...
        logAndRun(`sudo spctl --add '${outputFile}' --label "CometCord Installer"`);
...
try {
    execFileSync(installerBin, args, {
        ...
        env: {
            ...process.env,
            CometCord_USER_DATA_DIR: BASE_DIR,
            CometCord_DEV_INSTALL: "1"
        }
    });
} catch {
