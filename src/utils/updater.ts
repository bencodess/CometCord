...
export async function checkForUpdates() {
    changes = await Unwrap(CometCordNative.updater.getUpdates());
...
export async function update() {
    ...
    const res = await Unwrap(CometCordNative.updater.update());

    if (res) {
        isOutdated = false;
        if (!await Unwrap(CometCordNative.updater.rebuild()))
...
export const getRepo = () => Unwrap(CometCordNative.updater.getRepo());
